...
import (
	...
	"github.com/jirihnidek/rhsm-server/pkg/version"
...
const (
	socketPath     = "/run/rhsm/com.redhat.rhsm"
	pidFilePath    = "/run/rhsm/rhsm-server.pid"
	socketDirPerms = 0755
	socketPerms    = 0660
	pidFilePerms   = 0644
...
func acquirePIDLock() (func(), error) {
	...
	if err = syscall.Flock(int(pidFile.Fd()), syscall.LOCK_EX|syscall.LOCK_NB); err != nil {
		...
		if errors.Is(err, syscall.EWOULDBLOCK) {
			errMsg := "another instance of rhsm-server is already running"
			if pid := mustReadFile(pidFile); pid != "" {
...
func run() error {
	...
	rhsmRegistry := govarlink.NewRegistry(&govarlink.RegistryOptions{
		...
		URL:     "https://github.com/jirihnidek/rhsm-server",
	})

	rhsmHandler := rhsmapi.Handler{Backend: rhsmBackend}
	rhsmHandler.Register(rhsmRegistry)

	varlinkServer := &govarlink.Server{Handler: rhsmRegistry}
	...
	slog.Info("rhsm-server starting", "version", version.Version)
	...
	slog.Info("rhsm-server stopped")
	return nil
}

// beforeAction performs setup actions before the main action
func beforeAction(ctx context.Context, cmd *cli.Command) (context.Context, error) {
	...
	if unknownLogLevel {
		slog.Warn("unknown log level", "level", logLevelStr)
	}
	slog.Info("Log level set to", "level", logLevelStr)

	return ctx, nil
}

// mainAction handles the main server action with context and command
func mainAction(ctx context.Context, cmd *cli.Command) error {
	...
	if err := run(); err != nil {
		slog.Error("rhsm-server error", "error", err)
		os.Exit(ExitCodeErr)
	}
	return nil
}
...
func main() {
	rootCmd := &cli.Command{
		Name:        "rhsm-server",
		Usage:       "RHSM server providing Varlink API",
		Description: "Starts the RHSM server with Varlink API support",
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:  "log-level",
				Value: "info",
				Usage: "Set the log level to `LEVEL`",
			},
		},
		Before: beforeAction,
		Action: mainAction,
